    ...
    var measurementMeta: LogoPrimitiveMeta? {
        ...
        case .convertMeasure:
            return LogoPrimitiveMeta(
                ...
                description: "Converts a measurement or numeric value between compatible units (e.g., m to km, kg to g, c to f).",
                localizedDescriptionKey: "logo.doc.convertmeasure",
                source: .zago,
                parameters: [
                    LogoPrimitiveParameter(name: "valueOrMeasurement", required: true, description: "A numeric value or a measurement object [value unit].", example: "1000"),
                    LogoPrimitiveParameter(name: "fromUnitOrTargetUnit", required: true, description: "Source unit (if value is numeric) or target unit (if first arg is measurement).", example: "m"),
                    LogoPrimitiveParameter(name: "toUnit", required: false, description: "Target unit for conversion (when first arg is numeric).", example: "km"),
                ],
                examples: [
