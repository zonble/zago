...
public struct IPCServerLimits: Sendable {
    ...
    public init(maxConnections: Int = 16, maxPayloadBytes: Int = 1_048_576, clientIdleTimeout: TimeInterval = 30, clientWriteTimeout: TimeInterval = 5) {
...
public enum ZagoIPCServerFactory {
    public static func make(socketPath: String? = nil, sessionToken: String? = nil, limits: IPCServerLimits = .init()) -> any ZagoIPCServer {
        #if os(Windows)
...
extension ZagoIPCServer {
    ...
    private func dispatch(_ parsed: ZagoIPCParsedRequest, parser: JSONRPCParser) -> JSONRPCResponse {
        ...
        case .getBuffers:
            ...
            do {
                return parser.makeSuccessResponse(for: parsed, response: .buffers(try dataSource.ipcServerGetBuffers(self)))
            } catch { return requestFailure(error, parser: parser, request: parsed) }

        case .getText(let bufferTarget, let startLine, let endLine):
            guard let dataSource else { return parser.makeFailureResponse(code: 500, message: "Target data source unhandled", request: parsed) }
            do {
                guard let result = try dataSource.ipcServer(self, textFor: bufferTarget, startLine: startLine, endLine: endLine) else {
        ...
        case .getCursor(let bufferTarget):
            ...
            do {
                ...
                return parser.makeSuccessResponse(for: parsed, response: .cursor(line: cursor.line, column: cursor.column, visualCol: cursor.visualCol, mode: cursor.mode))
            } catch { return requestFailure(error, parser: parser, request: parsed) }

        case .showPreview(let client, let reason, let affectedFiles):
            guard let delegate else { return parser.makeFailureResponse(code: 500, message: "Target delegate unhandled", request: parsed) }
            do {
                guard try delegate.ipcServer(self, showPreviewFor: client, reason: reason, affectedFiles: affectedFiles) else {
                    return parser.makeFailureResponse(code: 409, message: "Failed to push proposal into queue or depth limit exceeded", request: parsed)
                }
        ...
        case .executeLogo(let script, let mode):
            ...
            do {
                ...
                return parser.makeSuccessResponse(for: parsed, response: .logo(success: result.success, result: result.result, error: result.error))
            } catch { return requestFailure(error, parser: parser, request: parsed) }

        case .getHistory(let limit):
            guard let dataSource else { return parser.makeFailureResponse(code: 500, message: "Target data source unhandled", request: parsed) }
            do { return parser.makeSuccessResponse(for: parsed, response: .history(try dataSource.ipcServer(self, historyWithLimit: limit))) }
            catch { return requestFailure(error, parser: parser, request: parsed) }
        }
    }
    ...
    private func requestFailure(_ error: Error, parser: JSONRPCParser, request: ZagoIPCParsedRequest) -> JSONRPCResponse {
        ...
        case .timedOut: return parser.makeFailureResponse(code: 408, message: "Editor request timed out", request: request)
