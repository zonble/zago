...
- `:version`: `zago` editor & engine version (e.g. `"1.4.0"`).
...
| `FORMAT.NAME` | `FORMATNAME` | `FORMAT.NAME given [middle] family [style] [locale]` or `FORMAT.NAME [propList]` | Formats person name components (given, middle, family, prefix, suffix, nickname) according to localized cultural conventions (macOS only) | `FORMAT.NAME "Arthur "Conan "Doyle "long`, `FORMAT.NAME "Steve "Jobs "abbreviated` |
...
| `CONVERT.CALENDAR` | - | `CONVERT.CALENDAR date targetCal [sourceCal] [format]` | Converts dates across calendar systems (Gregorian, ROC, Japanese Wareki, Buddhist, etc.) | `CONVERT.CALENDAR "2026-08-19 "roc`, `CONVERT.CALENDAR "民國115年8月19日 "gregorian` |
...
| `SORT.LOCALIZED` | - | `SORT.LOCALIZED [ASC\|DESC] [locale] data [template]` | Sorts list, array, or word using natural localized order and optional locale | `SORT.LOCALIZED ["file10.txt "file2.txt "file1.txt]`, `SORT.LOCALIZED "DESC "de_DE ["ä "b "a]` |
...
3. **Foundation Formatting Family (`FORMAT.xxx`)**:
   ...
   SHOW FORMAT.NAME "Arthur "Conan "Doyle "long            ; "Arthur Conan Doyle"
   SHOW FORMAT.NAME "Steve "Jobs "abbreviated              ; "S. Jobs"
   SHOW FORMAT.NAME [given "Arthur middle "Conan family "Doyle] "short ; "Arthur"
