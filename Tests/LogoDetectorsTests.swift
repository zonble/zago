...
@Suite struct LogoDetectorsTests {
    ...
    @Test func detectsURLsAndEmailsAsLogoLists() {
        let engine = LogoEngine()
        var index = 0
        let urls = engine.evaluateExpression(["DETECT.URL", "Visit https://example.com and https://swift.org"], index: &index)
    ...
    @Test func detectsPhonesDatesAndAddresses() {
        ...
        index = 0
        let address = engine.evaluateExpression(["DETECT.ADDRESS", "Ship to 1600 Pennsylvania Avenue NW, Washington, DC"], index: &index)
